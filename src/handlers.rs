...
use crate::{payloads, payloads::groups::{GroupResult, NewGroupForm}, AppState};
...
/**
    This Api is about add a new user and a new group
    for quick init
    TODO: will discuss
*/
pub async fn create_user_and_group(
  ...
  let transaction_rs: Result<GroupResult, diesel::result::Error> = conn.transaction(|conn| {
    ...
    diesel::insert_into(schema::participants::table)
        .values((
          schema::participants::user_id.eq(user_result.id),
...
pub async fn get_user_groups(
  ...
  tracing::info!("User found: user_id = {}, user_code = {}", user.id, user.user_code);
  ...
  let user_groups = participants::table
      ...
      .select((groups::id, groups::name, groups::group_code, groups::expired_at))
  ...
  tracing::info!("Groups found for user_id {}: {}", user_id, user_groups.len());
...
pub async fn create_group_with_user(
    ...
    let user_exists = users::table
        ...
        .optional()
        .map_err(|err| {
            tracing::error!("Error checking user_id {}: {:?}", new_group_req.user_id, err);
